...
`..` | 再帰処理 | 3.6節
`:` | 1) 配列のスライス、2）関数定義 | [3.1節](./section03-filter.md#31-角カッコ)、10.4節
`;` | 1）関数引数のセパレータ、2）`def`文の終端 | 4.1節、10.4節
`?` | エラーを無視 | 3.4節、10.2節
`[]` | 1) 配列要素指定（`[n]`）、2) 配列および文字列スライス（`[n:m]`）、3) プロパティ指定（`["property"]`）、4）イテレータ（`[]`）、5）配列化(`[...]`) | 1.2節、3.1節
`{}` | オブジェクト生成 | 3.5節
<code>&#124;</code>| 1）パイプ（連続フィルタリング処理）、2）定数定義の終端 | 3.3節、[10.3節](./section10-control.md#103-変数)
...
本節には関数をまとめて示します。変数定義や関数定義のような関数らしからぬ記法も含んでいます。数学関数は別途付録C.5に掲載しています。ソートなどの値の大小比較でデータ型が混在しているときの順序は次のとおりです。
...
関数 | 引数の数 | 機能 | 参照
-----|--------|------|-------
<code>... as $identifier &#124;</code> | 該当しない | 変数`$identifier`を定義する | 10.3節
...
`def name: 式;` | 該当しない | 関数を`name`を定義する | 10.4節
