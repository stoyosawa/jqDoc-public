...
本付録では、本書で取り上げたjqコマンドオプション（付録C.1）、`.`などの特殊記号（付録C.2）、演算子（付録C.3）、一般的な関数（付録C.4）、数学関数（付録C.5）、定数（付録C.6）をまとめて表に示します。項目はアルファベット順（記号はASCIIコード順）に並べています。

本書は、[jqマニュアル v1.6](https://stedolan.github.io/jq/manual/v1.6/ "LINKS")記載の機能をすべて取り上げているわけでありません。以下の表で未記述のものは、マニュアルを参照してください。

関数引数の数を示した表で、関数引数が0とは、パイプからのみ入力を受けつけるという意味です。範囲で示したときは、引数が機能に応じて可変です。たとえば、`range`関数（5.5節）は`range(10)`、`range(10;20)`、`rqnge(10;20;2)`のように3通りの使いかたがあるので、1～3と示しています。関数引数がふたつ以上のときは、`f(a; b)`のようにセミコロン（`;`）で区切ります。

### C.1 コマンドオプション

本節にはコマンドオプションをまとめて示します。ハイフンとアルファベット1文字からなるショートカットがあるものは表2列目に記載しました。
...
`--argjson <name> <value>` | | 変数`<var>`に値`<value>`を定義する（値は表記とおりの型になる）| 10.3節
...
`--slurpfile <var> <file>`| | 変数`<var>`に`<file>`の中身を代入する | 10.3節
...
本節には関数をまとめて示します。変数定義や関数定義のような関数らしからぬ記法も含んでいます。数学関数は別途付録C.5に掲載しています。

表C.4 関数

関数 | 引数の数 | 機能 | 参照
-----|--------|------|-------
<code>... as $identifier &#124;</code> | NA | 変数を定義する | 10.3節
...
`if-then-else-end` | NA | if文。`else`から`if`を続けるときは`elif`を用いる。 | 10.1節
...
`map` | 1 | 配列要素を引数の操作で変換した配列を返す | 7.3節
`map_values` | 1 | オブジェクトの値を引数の操作で変換する | 7.3節
`match` | 1 | （正規表現）マッチした部分文字列をオブジェクトで返す | 6.2節
`max` | 0 | 配列の最大値を返す（`sort`参照） | 4.4節
`max_by` | 0 | オブジェクトの値の最大値を返す（`sort_by`参照） | 4.4節
`min` | 0 | 配列の最小値を返す（`sort`参照） | 4.4節
`min_by` | 0 | オブジェクトの値の最小値を返す（`sort_by`参照） | 4.4節
...
`hypot`       |2| ユークリッド距離を求める（sqrt(x<sup>2</sup> + y<sup>2</sup>)）| 5.4節
`log`         |0| （底がeの）対数関数（exponential function with base e; log<sub>e</sub>xまたはln x）| 5.4節
`log10`       |0| （底が10の）対数関数（exponential function with base 10; log<sub>10</sub>xまたはlog x）| 5.4節
`log2`        |0| （底が2の）対数関数（exponential function with base 2; log<sub>2</sub>x）| 5.4節
